...
VOID ixheaacd_mps_synt_calc(ia_mps_dec_state_struct *self) {
  ...
  VOID(*ixheaacd_mps_synt_out_calc_pointer) (WORD32 resolution, FLOAT32 *out, FLOAT32 *state, const FLOAT32 *filter_coeff);
  ...
  if (self->qmf_band_count == 32)
  {
    for (ch = 0; ch < self->out_ch_count; ch++) {
      ...
      for (ts = 0; ts < self->time_slots; ts++) {
		...
		ixheaacd_mps_synt_pre_twiddle_dec(&self->qmf_out_dir[ch][ts][0].re, ixheaacd_ldmps_pre_twid, fin_re, fin_im,
					resolution);
