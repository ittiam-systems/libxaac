...
static WORD32 freq_res_table[] = {0, 23, 15, 12, 9, 7, 5, 4};

static WORD32 HRTF_freq_res_table[][8] = {{0, 28, 20, 14, 10, 7, 5, 4},
                                          {0, 13, 13, 8, 7, 4, 3, 3}};

static TREEPROPERTIES tree_property_table[] = {
        ...
        case 2:
          ...
          for (i = 0; i < config->num_ott_boxes_AT; i++) {
            ...
            if (config->bs_ott_mode_lfe_AT[i]) {
              ...
              config->bs_ott_bands_AT[i] = freq_res_table[config->bs_freq_res];
            }
          }
...
IA_ERRORCODE ixheaacd_ld_spatial_specific_config(
  ...
  if (config->bs_tree_config != 15) {
    config->num_ott_boxes =
        tree_property_table[config->bs_tree_config].num_ott_boxes;
    config->num_ttt_boxes =
        tree_property_table[config->bs_tree_config].num_ttt_boxes;
    config->num_input_channels =
        tree_property_table[config->bs_tree_config].num_input_chan;
    config->num_output_channels =
        tree_property_table[config->bs_tree_config].num_output_chan;
    for (i = 0; i < MAX_NUM_OTT; i++) {
      config->ott_mode_lfe[i] =
          tree_property_table[config->bs_tree_config].ott_mode_lfe[i];
    }
  }
  ...
  for (i = 0; i < config->num_ott_boxes; i++) {
    if (config->ott_mode_lfe[i]) {
      ...
      config->bs_ott_bands[i] = freq_res_table[config->bs_freq_res];
    }
  }
  ...
  for (i = 0; i < config->num_ttt_boxes; i++) {
    ...
    if (config->bs_ttt_dual_mode[i]) {
      ...
      config->bs_ttt_bands_high[i] = freq_res_table[config->bs_freq_res];
    } else {
      config->bs_ttt_bands_low[i] = freq_res_table[config->bs_freq_res];
    }
  }
  ...
  if (config->bs_3D_audio_mode) {
    ...
    if (config->bs_3D_audio_HRTF_set == 0) {
      ...
      config->HRTF_num_band = HRTF_freq_res_table[0][config->bs_HRTF_freq_res];
      config->HRTF_num_phase = HRTF_freq_res_table[1][config->bs_HRTF_freq_res];
