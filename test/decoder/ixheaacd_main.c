...
if (mp4_supp) {
  ...
  number_of_frames = meta_info.ia_mp4_stsz_entries;
}
...
int main(WORD32 argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!strncmp((const char *)argv[i], "-ifile:", 7)) {
      ...
      SIZE_T len = strlen(pb_arg_val);
      if (len > 4 && strcmp(pb_arg_val + len - 4, ".mp4") == 0) {
        mp4_supp = 1;
      }
      else {
        mp4_supp = 0;
